...
with tab1 :
    ...
    if periode == "1 mois":
        tableau = Analyse(dataframes, dict_assets_names).KPI_1month()
    elif periode == "6 mois":
        tableau = Analyse(dataframes, dict_assets_names).KPI_6month()
    elif periode == "1 an":
        tableau = Analyse(dataframes, dict_assets_names).KPI_1year()
    elif periode == "5 ans":
        tableau = Analyse(dataframes, dict_assets_names).KPI_5year()
...
with tab2:
    ...
    try :
        color_style, nb_part, tx_rendement, rendement, tx_rentabilite, rentabilite, tx_renta_lower, renta_lower, tx_renta_upper, renta_upper = Projection(montant, duree, asset, df_prophet, forecast).unit_projection()
        st.write(f'{color_style}')
